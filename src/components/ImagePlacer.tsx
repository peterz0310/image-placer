...
export default function ImagePlacer() {
  ...
  const handleOverlayUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      const generateUniqueName = (baseName: string, existingLayers: Layer[]): string => {
        ...
        let uniqueName;
        do {
          uniqueName = `${nameWithoutExt} (${counter})${extension ? '.' + extension : ''}`;
          counter++;
  ...
  const exportProjectZIP = async () => {
    ...
    try {
      ...
      ProjectExporter.downloadBlob(zipBlob, `image-placer-project-${timestamp}.zip`);
    } catch (error) {
  ...
  const loadProject = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.name.endsWith('.zip')) {
      ...
      try {
        ...
        const baseImageFile = zipContent.files[`assets/${projectData.base.name}`];
        if (baseImageFile) {
        ...
        <div className="flex gap-2">
          ...
          {project && (
            <div className="flex bg-gray-700 rounded">
              <button
                onClick={() => setCanvasState(prev => ({ ...prev, transformMode: 'normal' }))}
              ...
              <button
                onClick={() => setCanvasState(prev => ({ ...prev, transformMode: 'skew' }))}
      ...
      <div className="flex-1 flex">
          ...
          <div className="w-80 bg-gray-100 border-r flex flex-col">
            <div className="p-4">
              ...
              <div className="mb-4 p-3 bg-white rounded border">
                <div className="font-medium text-sm text-gray-800">Base Image</div>
        ...
        <div className="flex-1 bg-gray-50 flex items-center justify-center">
          {project ? (
            ...
            <div className="text-center text-gray-500">
              ...
              <p>Start by loading a base image to begin creating your composition</p>
            </div>
          )}
        </div>
        ...
        {project && (
          <div className="w-80 bg-gray-100 border-l">
            ...
            <div className="p-4 border-b bg-gray-50">
              <div className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full ${
                  canvasState.transformMode === 'skew' ? 'bg-orange-500' : 'bg-blue-500'
                }`}></div>
                <span className="text-sm font-medium text-gray-900">
                  {canvasState.transformMode === 'skew' ? 'Skew Mode' : 'Transform Mode'}
                </span>
              </div>
            ...
            <LayerProperties
              layer={canvasState.selectedLayerId 
                ? project.layers.find(l => l.id === canvasState.selectedLayerId) || null
                : null
              }
              onUpdateLayer={(updates) => {
                if (canvasState.selectedLayerId) {
                  ...
                  const updatedLayer = {
                    ...project.layers.find(l => l.id === canvasState.selectedLayerId)!,
                    ...updates
                  };
